...
rows = File.readlines('import_data/user_data.csv')
...
rows.each do |row|
	...
	db.execute('INSERT INTO users (img, name, email, phone, department_id) VALUES (?,?,?,?,?)', [columns[0], columns[1], columns[2], columns[3], department_id])
end
